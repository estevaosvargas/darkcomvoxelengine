...
public class FlyCamera : MonoBehaviour
{
    ...
    public Block currenBlock;

    void Update()
    {
        currenBlock = World.Instance.GetTileAt((int)transform.position.x, (int)transform.position.y, (int)transform.position.z);
        ...
        {
            if (Input.GetTouch(0).position.magnitude > 0)
            {
                newPosition += transform.forward * speedMultiplier;
            }
        }
        ...
        {
            newPosition += -transform.right * speedMultiplier;
        }
